...
/*
  request - make external request, broadcast response
  scheduled message - save message, add to scheduler

  on scheduled request
    run throw handlers (don't save)

  on startup:
    load all requests that have schedules and add to scheduler
 */
...
function Connection(socket) {
  ...
  return {
    send: function(o) { socket.send(JSON.stringify(o)); },
