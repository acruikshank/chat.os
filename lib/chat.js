...
module.exports.manage = function( socketServer ) {
  ...
  socketServer.on('connection', function(ws) {
    ...
    ws.on('message', function(message) {
      ...
      if ( message.schedule && message.name )
        scheduleMessage(connection, message);
    ...
    ws.on('disconnect', function() {
      ...
      if ( connection.room && chatrooms[connection.room] ) {
        ...
        handleMessage( connection, {type:'disconnected',participants:chatrooms[connection.room].length});
      }
    })
  ...
  function scheduleMessage( connection, message ) {
    ...
    if ( message.persist === false ) return handled();
  ...
  function identify( connection, message ) {
    ...
    handleMessage( connection, {type:'joined',participants:chatrooms[connection.room].length} );
  ...
  function replay( connection, message ) {
    ...
    if ( message.type ) crit.type = typeof message.oftype !== 'string' ? {$in:message.oftype} : message.oftype;
  ...
  function rollcall( connection, message ) {
    var rollcall = {type:'rollcall', from:connection.identity, 
                    timestamp:new Date().getTime(), room:connection.room };
    rollcall.participants = (chatrooms[connection.room] || []).map(function(c) { return c.identity; } )
    connection.send( rollcall );
    handled();
  }
  ...
  function defaultHandler( connection, message, done ) {
    ...
    if ( message.persist === false ) return done();
...
function Connection(socket) {
  ...
  return {
    ...
    identity: {
      email: socket.handshake.identity.email,
      name: socket.handshake.identity.name,
      picture: socket.handshake.identity.picture },
    room: null,
    id: id
  }
}
